...
def fitness(problem: Problem, cost_weight: float, priority_weight: float, cost: float, priority: int, not_served_count: int = 0):
    ...
    # Calcular el máximo coste y la máxima prioridad
    max_cost = get_max_cost(problem)
    max_priority = get_max_priority(problem)
    min_priority = len(problem.orders)  # El mínimo es la cantidad de pedidos en el problema

    # Normalizar el coste y la prioridad
    normalized_cost = cost / max_cost if max_cost > 0 else 0
    normalized_priority = (priority - min_priority) / (max_priority - min_priority) if (max_priority - min_priority) > 0 else 0

    # Calcular el fitness con los pesos y penalizaciones
    return (cost_weight * normalized_cost + priority_weight * normalized_priority + 5 * not_served_count) * 100
