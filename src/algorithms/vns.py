...
def vns(problem: Problem, iterations: int, alpha: float, beta: float, time_limit: int = float('inf'), log: bool = False, initial_solution: list = None):
    """
    ...
    while iter in range(iterations) and time.time() - start_time < time_limit:
        it_start = time.time()
    ...
    return best_solution, best_solution_value, best_solution_fitness, stats, execution_time, inicial_routes, inicial_solution_fitness
